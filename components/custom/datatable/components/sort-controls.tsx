...
export default function DataTableSortControls<TData>({ table }: DataTableSortControlsProps<TData>) {
  ...
  const allColumns = table.getAllColumns().filter(column => column.getCanSort());
  ...
  const addSort = () => {
    ...
    const availableColumns = allColumns.filter(column => !sorts.some(sort => sort.id === column.id));
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm" className="h-8">
          ...
          Sort
          {sorts.length > 0 && <span className="ml-1 rounded-full bg-primary w-5 h-5 text-xs flex items-center justify-center text-primary-foreground">{sorts.length}</span>}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-[280px] sm:w-[400px] p-0" align="end">
        <div className="p-4">
                    ...
                    {sorts.map((sort, index) => (
                      <Draggable key={index} draggableId={`sort-${index}`} index={index}>
                        {(provided: DraggableProvided) => (
                          <div ref={provided.innerRef} {...provided.draggableProps} className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                            <div {...provided.dragHandleProps} className="flex h-4 w-4 items-center justify-center mb-1 sm:mb-0">
                            ...
                            <div className="flex flex-1 flex-col sm:flex-row w-full sm:w-auto gap-2">
                              ...
                              >
                                <PopoverTrigger asChild>
                                  <Button variant="outline" role="combobox" className="flex-1 justify-between h-9 w-full sm:min-w-[200px]">
                              ...
                              <div className="flex items-center gap-2">
                                <Select value={sort.desc ? 'desc' : 'asc'} onValueChange={value => updateSort(index, sort.id, value === 'desc')}>
                                ...
                                <Button variant="ghost" size="icon" onClick={() => removeSort(index)} className="h-8 w-8 p-0 hover:bg-destructive/10">
          ...
          <div className="flex justify-between mt-4">
            ...
            {sorts.length > 0 && (
              <Button variant="outline" size="sm" className="ml-auto text-xs" onClick={resetSorting}>
                Reset
              </Button>
            )}
          </div>
